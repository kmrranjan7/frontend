...
export function SavedJobsHeaderButton() {
  ...
  function removeSavedJob(id: string) {
    const next = savedJobs.filter((job) => job.id !== id);
    writeSavedJobs(next);
    setSavedJobs(next);
  }
  ...
  return (
    <div ref={popoverRef} className="relative shrink-0">
      ...
      {open ? (
        <section className="fixed right-2 left-2 top-[14px] z-[120] overflow-hidden rounded-lg border border-rose-100 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.2)] sm:absolute sm:right-0 sm:left-auto sm:top-10 sm:w-[320px]" aria-label="Saved jobs">
          <header className="flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-rose-50 to-white px-2.5 py-1.5"><div><strong className="block text-[10px] text-slate-900">Saved updates</strong><span className="text-[8px] text-slate-500">Available on this browser</span></div><div className="flex items-center gap-1.5">{savedJobs.length ? <Link href="#" onClick={(event) => { event.preventDefault(); clearSavedJobs(); }} className="text-[9px] font-semibold text-red-600 underline underline-offset-2 hover:text-red-800">Clear All</Link> : null}<button type="button" onClick={() => setOpen(false)} aria-label="Close saved updates" className="grid size-4 place-items-center text-[10px] leading-none text-slate-500 hover:text-slate-800"><span className="-translate-y-px">×</span></button></div></header>
          <div className="max-h-[min(310px,60vh)] overflow-y-auto p-1 [scrollbar-width:thin]">
            {savedJobs.length ? savedJobs.map((job) => <article key={job.id} className="mb-1 rounded-lg border border-indigo-100/80 bg-white px-1.5 py-1 shadow-[0_6px_14px_rgba(15,23,42,0.06)] transition hover:border-indigo-200 hover:shadow-[0_8px_18px_rgba(99,102,241,0.12)]"><div className="flex items-start justify-between gap-2"><div className="flex min-w-0 items-start gap-1.5"><span className="relative mt-0.5 inline-flex size-[18px] shrink-0 items-center justify-center rounded-full bg-rose-100 text-rose-600"><Icon name="heart" size={9} filled /><span className="absolute -bottom-2.5 left-1/2 h-2 w-px -translate-x-1/2 bg-rose-200/80" aria-hidden="true" /></span><div className="min-w-0"><Link href={job.savedHref || `/jobs?q=${encodeURIComponent(job.title)}`} prefetch={false} onClick={() => setOpen(false)} className="block truncate text-[10px] font-semibold text-slate-800 underline-offset-2 hover:text-indigo-700 hover:underline">{job.title}</Link><span className="block text-[8px] font-medium text-slate-500">{job.savedLabel === "RESULT" ? "Released" : "Last date"}: {formatDate(job.savedLabel === "RESULT" ? job.startDate : job.lastDate)}</span></div></div><div className="flex shrink-0 items-start gap-1"><span className={`max-w-16 truncate rounded-full px-1.5 py-[2px] text-[7px] font-bold uppercase tracking-[0.08em] ${getBadgeStyle(job.savedLabel || badgeLabel(job.department))}`}>{job.savedLabel || badgeLabel(job.department)}</span><button type="button" onClick={() => removeSavedJob(job.id)} aria-label={`Remove ${job.title} from saved updates`} title="Remove saved update" className="grid size-[14px] place-items-center text-[9px] leading-none text-rose-500 hover:text-rose-700"><span className="-translate-y-px">×</span></button></div></div></article>) : <div className="px-4 py-6 text-center"><span className="inline-flex text-rose-400"><Icon name="heart" size={16} /></span><strong className="mt-1.5 block text-[9px] text-slate-700">No saved updates yet</strong><span className="mt-1 block text-[8px] text-slate-500">Use the heart on a listing card.</span></div>}
          </div>
        </section>
      ) : null}
    </div>
